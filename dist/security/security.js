...
class SecurityManager {
     ...
     */
    async getSigningKeyForTransfer() {
        const data = this.api.createAuthenticatedRequestData({
            format: 'jwk',
            type: 'signing',
        });
    ...
    async getMasterEncryptionKey() {
        const data = this.api.createAuthenticatedRequestData({});
        try {
            const response = await this.api.post('/security/get_master_encryption_key', data);
            return response;
        }
        catch (error) {
    ...
    async getVerifiedKeys() {
        const data = this.api.createAuthenticatedRequestData({});
        try {
            const response = await this.api.post('/security/get_verified_keys', data);
            return response;
        }
        catch (error) {
     ...
     */
    async decryptSigningKeyWithCode(signingKeyData, code) {
        // Parse the private_key JSON string containing encrypted components
        let encryptedKey;
        try {
            encryptedKey = JSON.parse(signingKeyData.private_key);
        }
        catch {
        ...
        try {
            ...
            throw new Error(`Failed to decrypt signing key with code — check the 6-digit code: ${error instanceof Error ? error.message : error}`);
        }
    }
    /**
    ...
    setPrivateKeyFromJWK(jwk) {
        try {
            ...
            throw new Error(`Failed to import private key from JWK: ${error instanceof Error ? error.message : error}`);
        }
        this.aesKeyCache.clear();
    }
    /**
    ...
    exportPrivateKeyAsJWK() {
        ...
        try {
            const jwk = this.rsaPrivateKey.export({ format: 'jwk' });
            return jwk;
        }
        catch (error) {
            throw new Error(`Failed to export private key: ${error instanceof Error ? error.message : error}`);
        }
    }
}
