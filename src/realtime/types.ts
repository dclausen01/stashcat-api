...
export interface RealtimeEvents {
  ...
  // ─── Auth / Verbindungs-Events ────────────────────────────────────────────
  /** Server bestätigt neue Socket-Verbindung für ein Gerät */
  new_device_connected: (data: { device_id: string; ip_address: string }) => void;
  /** Eine andere Verbindung für dieses Gerät wurde getrennt */
  device_disconnected: (data: { device_id: string }) => void;
...
export interface MessageSyncPayload {
  id: number;
  text: string;               // Klartext oder verschlüsselter Hex-String (wenn encrypted=true)
  conversation_id: number;    // 0 wenn Channel-Nachricht
  channel_id: number;         // 0 wenn Conversation-Nachricht
  thread_id: number;
  hash: string;
  verification: string;
  ...
  time: number;               // Unix-Timestamp (Sekunden)
  micro_time: number;         // Unix-Timestamp (Mikrosekunden)
  sender: {
    id: string;
    first_name: string;
    last_name: string;
  ...
  kind: string;               // z.B. 'text'
  type: string;
  location: unknown | null;
  reciever: unknown[];        // Typo im API: 'reciever' (nicht 'receiver')
  files: unknown[];
  likes: number;
  liked: boolean;
  flagged: boolean;
  ...
  unread: boolean;
  encrypted: boolean;
  iv: string | null;          // AES-IV (hex) wenn encrypted=true
